export const samples = {
  ...
  unknown: {
    ...
    status: "Needs human decision · low confidence",
...
export function specimenMarkup(result) {
  ...
  return `<div class="result-top"><div><p class="eyebrow">Analysis record</p><p><b>${esc(result.file)}</b></p></div><span class="badge${result.warning ? " warning" : ""}">${esc(result.badge)}</span></div><dl class="result-grid"><dt>Source group</dt><dd>${esc(result.source)}</dd><dt>Original time</dt><dd>${esc(result.original)}</dd><dt>Proposed time</dt><dd>${esc(result.proposed)}</dd><dt>Evidence</dt><dd>${esc(result.inference)}</dd><dt>Finding</dt><dd>${esc(result.status)}</dd><dt>CSV action</dt><dd>${esc(result.action)}</dd></dl>`;
}
